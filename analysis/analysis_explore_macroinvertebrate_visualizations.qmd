...
# Tell googlesheets4 NOT to authenticate (for public sheets)
gs4_deauth()

## Get mock-up data from google drive link
macroinvertbeltSurveySummTBL <- 
  read_sheet("18q1i0qWmN3YLHXsK_SUpksyZyZ4Khqlg_OK6TlCZEVY", 
             sheet = "sampleevents")
...
Barplots showing the average macroinvertebrate density (y-axis), each year (x-axis), separately for open access and fisheries restricted areas (open access = orange, restrictions = blue).
...
## Barplot - Densities by group of interest (single survey)

Barplot showing macroinvertebrate densities (y-axis) by group of interest (x-axis, with different colors for each group). This is an example with a single sample event (i.e. survey), but the code could easily be adapted to show averages across surveys or sites.

```{r}
#| label: Barplot - Macroinvertebrate density by group of interest (single survey)
...
macroinvertbeltSingleSeTBL <- macroinvertbeltSurveySummTBL %>% 
  ...
  slice(1) %>% #Take the first row as an example of a single SE
...
values_all <- as.numeric(unlist(macroinvertbeltSingleSeTBL[interestCols], use.names = FALSE))

## Calculate total density across all groups (including zeros; "Other" already removed above)
total_density_indha <- sum(values_all, na.rm = TRUE)

## Keep only groups with non-zero density for the bar chart
keep <- values_all != 0
interestLabels_plot <- as.character(interestLabels[keep])
values_plot <- values_all[keep]

## Order bars by descending density, with "Other" always last
is_other <- interestLabels_plot == "Other"
order_idx <- c(
  order(values_plot[!is_other], decreasing = TRUE),         # non-Other sorted desc
  which(is_other)                                            # Other appended at end
)
# Re-index relative to the full vectors
non_other_idx <- which(!is_other)
other_idx     <- which(is_other)
order_idx_full <- c(non_other_idx[order(values_plot[!is_other], decreasing = TRUE)],
                    other_idx)

interestLabels_plot <- interestLabels_plot[order_idx_full]
values_plot         <- values_plot[order_idx_full]

# Set factor levels to lock in the display order
  ...
  layout(showlegend = FALSE,
         ...
         annotations = list(
           ...
           list(x = 0, y = 1.07,
                ...
                font = list(size = 12)),
           list(x = 1, y = 1.07,
                text = paste0("<b>Total density:</b> ",
                              format(round(total_density_indha, 1),
                                     nsmall = 1), " ind/ha"),
                showarrow = FALSE,
